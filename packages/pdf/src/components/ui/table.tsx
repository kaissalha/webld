...
const processTableChildren = (children: ReactNode, variant: TableVariant, zebraStripe: boolean): ReactNode => {
	...
	return Children.map(children, (child) => {
		...
		if (child.type === TableRow) return cloneElement(child as ReactElement<TableRowProps>, { variant });
		return child;
	});
};
...
export const Table = ({ children, style, variant = "line", zebraStripe = false, noWrap = false }: TableProps) => {
...
export const TableRow = ({ header, footer, stripe, children, style, variant = "line" }: TableRowProps) => {
	const theme = usePdfxTheme();
	const styles = useSafeMemo(() => createTableStyles(theme), [theme]);
	...
	if (stripe && !header && !footer) rowStyles.push(styles.rowStripe);
...
export const TableCell = ({
	...
	const theme = usePdfxTheme();
	const styles = useSafeMemo(() => createTableStyles(theme), [theme]);
	const cellStyles: Style[] = width !== undefined ? [styles.cellFixed, { width } as Style] : [styles.cell];
	...
	if (variant === "grid" && !_last) cellStyles.push(styles.cellGridBorder);
	else if (variant === "bordered" && !_last) cellStyles.push(styles.cellBorderedBorder);
	if (align) cellStyles.push({ textAlign: align } as Style);
